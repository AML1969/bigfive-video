"""Recompute WCAG contrast for every colour in bs3/palette.py. Exit code 1 if anything fails.
...
spec = importlib.util.spec_from_file_location("palette", Path(__file__).resolve().parents[1] / "bs3" / "palette.py")
...
for theme, bgs in P.WEB_BACKGROUNDS.items():
    ...
    check("modality share bar (.55)", ink, (block,), 3.0, 0.55)
    ...
    check("RADAR_WEB.main", P.RADAR_WEB[theme]["main"], bgs, 3.0)
