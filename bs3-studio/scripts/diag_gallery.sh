...
ls -1t "$HOME/bs3_data/web_jobs" | head -4
echo "--- server process"
for p in $(pgrep -f "bs3 w[e]b"); do ps -o pid=,lstart= -p "$p"; done
...
echo "--- web.log errors"
grep -aiE "error|exception|traceback|invalid|not allowed" "$HOME/bs3_data/logs/web.log" | tail -8 | cut -c1-220
