#!/usr/bin/env bash
# Does the running BS Profiler 3.0 server serve key-frame JPEGs to the gallery? Usage: check_gallery.sh [PORT]
set -uo pipefail
PORT="${1:-7880}"
f=$(ls -t "$HOME"/bs3_data/web_jobs/*/explain/key_*.jpg 2>/dev/null | head -1)
